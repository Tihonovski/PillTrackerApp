...
function initializeFirebase() {
    try {
        ...
        if (firebaseConfig.apiKey === "YOUR_API_KEY") { // Basic check if config wasn't replaced
             alert("שגיאה: פרטי התצורה של Firebase חסרים או לא הוחלפו בקובץ script.js!");
        } else {
             alert("שגיאה באתחול החיבור לשירות הענן. בדוק את פרטי התצורה בקוד ואת החיבור לאינטרנט.");
        }
...
async function handleAuthStateChange(user) {
    ...
    if (user) {
        ...
        if (userNameEl) userNameEl.textContent = `שלום, ${user.displayName || 'משתמש'}`;
...
async function signInWithGoogle() {
    ...
    try {
        ...
        alert(`שגיאה בהתחברות: ${error.message} (קוד: ${error.code})`);
    }
}
...
function addLogEntry(amount) {
    ...
    updateDisplay();
    if (lastLogTimeEl) { /* ... feedback ... */ lastLogTimeEl.textContent = `תועד ${countTaken} כדור(ים) ב-${formatTime(new Date(newLogEntry.timestamp))}`; lastLogTimeEl.classList.add('!text-green-600'); setTimeout(() => { if(lastLogTimeEl) lastLogTimeEl.classList.remove('!text-green-600'); }, 3000); }
}
...
function renderDashboard() {
     ...
     if (nextPurchaseDate) { const daysToPurchase = diffInDays(todayStart, nextPurchaseDate); if (daysToPurchase <= 0) { daysUntilPurchaseEl.textContent = '(הגיע הזמן לרכוש!)'; nextPurchaseDateEl.parentElement.classList.add('!bg-yellow-100', 'border', 'border-yellow-300'); nextPurchaseDateEl.classList.add('!text-orange-600'); } else { daysUntilPurchaseEl.textContent = `(בעוד ${daysToPurchase} ימים)`; nextPurchaseDateEl.parentElement.classList.remove('!bg-yellow-100', 'border', 'border-yellow-300'); nextPurchaseDateEl.classList.remove('!text-orange-600'); } } else { daysUntilPurchaseEl.textContent = ''; }
     const remainingEstimate = calculateEstimatedDaysRemainingFromPlan(currentPills, userData.planOverrides);
     daysRemainingEl.textContent = `${remainingEstimate.days === Infinity ? '∞' : remainingEstimate.days} ימים`;
     estimatedEndDateEl.textContent = remainingEstimate.endDate ? `(עד ${formatDate(remainingEstimate.endDate)})` : '(כבר אזל)';
     lastPurchaseInfoEl.textContent = `תאריך: ${formatDate(lastPurchaseDate)}`;
     if (userData.lastLogTimestamp) { const logDate = new Date(userData.lastLogTimestamp); lastLogTimeEl.textContent = `עודכן לאחרונה: ${formatDateTime(logDate)}`; } else { lastLogTimeEl.textContent = ''; }
...
function renderHistoryPage() {
    ...
    if (!historyTableBody) { historyContent.innerHTML = `<table id="history-table" class="w-full border-collapse mt-4"><thead><tr><th class="border p-2 text-right bg-gray-100 font-semibold">תאריך ושעה</th><th class="border p-2 text-right bg-gray-100 font-semibold">כמות שנלקחה</th><th class="border p-2 text-center bg-gray-100 font-semibold">פעולה</th></tr></thead><tbody id="history-table-body"></tbody></table>`; historyTableBody = document.getElementById('history-table-body'); } else { historyTableBody.innerHTML = ''; }
    ...
    sortedLog.forEach(entry => { const row = historyTableBody.insertRow(); const timestamp = new Date(entry.timestamp); row.insertCell().textContent = formatDateTime(timestamp); row.insertCell().textContent = entry.amount; const actionCell = row.insertCell(); const deleteBtn = document.createElement('button'); deleteBtn.textContent = 'מחק'; deleteBtn.classList.add('delete-log-btn', 'bg-red-100', 'text-red-700', 'hover:bg-red-200', 'px-2', 'py-1', 'rounded', 'text-xs', 'border', 'border-red-200'); deleteBtn.dataset.logId = entry.id; deleteBtn.addEventListener('click', (event) => { const idToDelete = event.target.dataset.logId; if (confirm(`האם למחוק את התיעוד הזה (${entry.amount} כדורים)? הפעולה תוסיף את הכמות בחזרה למלאי.`)) { deleteLogEntry(idToDelete); } }); actionCell.appendChild(deleteBtn); });
}
...
function renderPlanPage() {
    ...
    if (!currentUser || !userData) { planContent.innerHTML = '<p class="text-center text-gray-500">יש להתחבר ולהגדיר נתונים תחילה.</p>'; return; }
    const planOverrides = userData.planOverrides || {};
    const today = new Date(); const todayStart = new Date(today.getFullYear(), today.getMonth(), today.getDate());
    const table = document.createElement('table'); table.id = 'plan-table'; table.classList.add('w-full', 'border-collapse'); const thead = table.createTHead(); const headerRow = thead.insertRow(); headerRow.innerHTML = `<th class="border p-2 text-right bg-gray-100 font-semibold">תאריך</th><th class="border p-2 text-right bg-gray-100 font-semibold">יום</th><th class="border p-2 text-center bg-gray-100 font-semibold">כמות מתוכננת</th>`; const tbody = table.createTBody();
...
function setupEventListeners() {
    ...
    // Initial Setup
    setupNowBtn.addEventListener('click', () => { purchaseModalTitle.textContent = "הגדרה ראשונית"; purchaseDateInput.value = getTodayDateString(); pillsAddedInput.value = ''; initialPillsCountInput.value = ''; initialCountSection.style.display = 'block'; purchaseModal.classList.add('active'); savePurchaseBtn.dataset.initialSetup = "true"; });
    // Record Purchase
    recordPurchaseBtn.addEventListener('click', () => { purchaseModalTitle.textContent = "תיעוד רכישה חדשה"; purchaseDateInput.value = getTodayDateString(); pillsAddedInput.value = ''; initialCountSection.style.display = 'none'; purchaseModal.classList.add('active'); delete savePurchaseBtn.dataset.initialSetup; });
    ...
    savePurchaseBtn.addEventListener('click', async () => { // Save Purchase Logic (Initial or Regular)
        ...
        if (!purchaseDateStr || isNaN(pillsAdded) || pillsAdded <= 0) { alert('אנא הזן תאריך וכמות כדורים חיובית שנוספו.'); return; }
        if (isInitialSetup) {
            const initialCount = parseInt(initialPillsCountInput.value, 10);
            if (isNaN(initialCount) || initialCount < 0) { alert('אנא הזן את הכמות הנוכחית הכוללת לאחר הרכישה.'); return; }
            ...
            userData = { lastPurchaseDate: purchaseDateStr, currentPillCount: initialCount, waitingPeriod: CONFIG.WAITING_PERIOD_DAYS, consumptionLog: [], planOverrides: {}, lastLogTimestamp: null };
             ...
             if (!userData) { console.error("Cannot record purchase: User data not loaded."); alert('שגיאה: נתוני משתמש לא נטענו. נסה לרענן.'); purchaseModal.classList.remove('active'); return; }
        ...
        else { console.error("Save verification failed."); alert("שגיאה בשמירת הנתונים או אימות השמירה."); }
    });
     // Log Consumption
    ...
    // Reset Data
     resetDataBtn.addEventListener('click', async () => { if (!currentUser) { alert("יש להתחבר תחילה."); return; } if (confirm('האם אתה בטוח שברצונך למחוק את כל נתוני המעקב עבור משתמש זה מהענן? הפעולה סופית!')) { const success = await deleteUserData(); if (success) { updateDisplay(); } } });
}
